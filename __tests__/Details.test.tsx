...
describe('Details', () => {
  it('mounts', () => {
    const { getByTestId } = render(<Details title={title} date={date} location={location} locationUrl={locationUrl} distance={distance} duration={duration} intensity={intensity} />);
    const details = getByTestId('details');
    expect(details).toBeInTheDocument();
  });
  it('renders the title', () => {
    const { getByText } = render(<Details title={title} date={date} location={location} locationUrl={locationUrl} distance={distance} duration={duration} intensity={intensity} />);
