...
describe('TrailInfo', () => {
  it('mounts', () => {
    const { getByTestId } = render(<TrailInfo trail={trail} link={link} rating={rating} date={date} />);
    expect(getByTestId('trail-info')).toBeInTheDocument();
  });
  it('has the trail and link', () => {
    const { getByText } = render(<TrailInfo trail={trail} link={link} rating={rating} date={date} />);
